...
from catalog.checkmarx_loader import get_triage_boost
...
def triage_score(candidate_paths: List[dict], sources: List[dict], run_id: str = "local", pipeline_cfg: dict = None, cx_seed: dict = None, business_ctx: dict = None) -> dict:
    """Score + filter paths, output prioritized[]."""
    ...
    for path in unique_paths:
        score, detail = _score_path(path, source_locations, source_fn_names, business_ctx.get("sensitive_flows", []))
        if cx_seed:
            cx_boost, cx_reason = get_triage_boost(
                cx_seed,
                path.get("entry_file", ""),
                int(path.get("source", {}).get("line", 0) or 0),
                path.get("sink", {}).get("file", ""),
                int(path.get("sink", {}).get("line", 0) or 0)
            )
            score += cx_boost
            if cx_reason:
                detail["cx_boost"] = cx_boost
                detail["cx_reason"] = cx_reason
                path["cx_matched"] = True
                
        if score < threshold:
            skipped += 1
            continue
        path["triage_score"] = score
