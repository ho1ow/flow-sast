...
def catalog_semgrep(state: PentestState) -> dict:
    """LangGraph node — run semgrep and return updated sources/sinks."""
    cfg = state["config"]
    semgrep_cfg = cfg.get("semgrep", {})
    ...
    # Save checkpoint
    checkpoint_save(
        {**state, "sources": sources, "sinks": sinks},
        state.get("checkpoint_dir", "pentest_logs/checkpoints"),
        run_id, "catalog_semgrep",
    )
