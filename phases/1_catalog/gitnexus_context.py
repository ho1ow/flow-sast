"""
phases/1_catalog/gitnexus_context.py
──────────────────────────────────────
LangGraph node: catalog_gitnexus_context

GitNexus có 2 main tools:
  gitnexus query   → Cypher graph queries   (gitnexus_runner.py)
  gitnexus context → Per-symbol context     (file này)

Node này chạy 2 bước:
  Step 1 (primary): gitnexus context --symbol <fn> --repo <path>
    → Lấy callers, callees, định nghĩa của các entry functions quan trọng
    → Enrich business context với actual code graph data

  Step 2 (supplementary): Đọc README/docs/OpenAPI để build business_context dict
    → system_type, description, critical_assets, business_flows

Output: state["business_context"] — dùng bởi tất cả analyze agents
via phases/_shared/prompt_loader.format_business_context()
"""
...
@safe_node("catalog_gitnexus_context")
def catalog_gitnexus_context(state: PentestState) -> dict:
    """LangGraph node — build business_context via gitnexus context + doc reading."""
    cfg = state["config"]
    ...
    cached = checkpoint_load(checkpoint_dir, run_id, "catalog_gitnexus_context")
    if cached:
        return cached
    ...
    # Pull entry functions từ catalog endpoints đã discover
    ...
    # ── Step 2: Doc reading (supplementary) ──────────────────────────────────
    repo = Path(repo_path)
    ...
    checkpoint_save(checkpoint_dir, run_id, "catalog_gitnexus_context", result)
    return result


# ── gitnexus context CLI ──────────────────────────────────────────────────────

def _gitnexus_context_symbols(
    binary: str, repo_path: str, symbols: List[str], timeout: int
) -> List[str]:
    """Run `gitnexus context --symbol <fn>` for each and collect results."""
    results = []
