...
from core.reliability import audit_log, checkpoint_save, safe_node
...
@safe_node("catalog_api")
def catalog_api(state: PentestState) -> dict:
    """LangGraph node — parse API routes and params across all frameworks."""
    cfg = state["config"]
    ...
    catalog_checksum = _compute_checksum(unique_endpoints, state.get("sources", []), state.get("sinks", []))
    ...
    checkpoint_save(
        {**state, "endpoints": unique_endpoints, "catalog_checksum": catalog_checksum},
        state.get("checkpoint_dir", "pentest_logs/checkpoints"),
        run_id, "catalog_api",
    )
