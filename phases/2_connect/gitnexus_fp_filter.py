...
from phases._shared.source_catalog import (
    TRUSTED_SOURCES,
    TEST_FILE_PATTERNS,
    READONLY_SINKS,
    is_trusted_source,
...
from phases._shared.sink_catalog import (
    SINK_SEVERITY_SCORE,
    get_category,
)
...
@safe_node("gitnexus_fp_filter")
def gitnexus_fp_filter(state: PentestState) -> dict:
    """
    LangGraph node — GitNexus pattern-based FP filter.
    Reads prioritized[], applies FP rules, returns filtered prioritized[].
    """
    cfg = state["config"]
    ...
    paths: List[dict] = state.get("prioritized", [])
    if not paths:
        return {}
