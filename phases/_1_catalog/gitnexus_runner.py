...
@safe_node("catalog_gitnexus")
def catalog_gitnexus(state: PentestState) -> dict:
    """LangGraph node — GitNexus Cypher-based catalog."""
    cfg = state["config"]
    ...
    # Resume check
    cached = checkpoint_load(checkpoint_dir, run_id, "catalog_gitnexus")
    if cached:
        audit_log(audit_dir, run_id, "catalog_gitnexus:resumed", {
            "custom_sinks": len(cached.get("custom_sinks", [])),
            "endpoints": len(cached.get("endpoints", [])),
        })
        return cached
    ...
    checkpoint_save(checkpoint_dir, run_id, "catalog_gitnexus", result)
